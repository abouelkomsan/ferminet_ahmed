...
def get_config():
  ...
  zeros = np.linspace(-lattice[:,0]/2, lattice[:,0]/2, 9)  
  complex_zeros = (zeros[:, 0] + 1j * zeros[:, 1])/(np.sqrt(2))
  cfg.network.psiformer_magfield.kwargs = {"lattice": lattice, "N_phi": 9,"zeros": complex_zeros,"rescale": 1.0}
