...
def get_config(momind):
  ...
  cfg.network.network_type = "psiformer_momind" if cfg.targetmom.mom is not None else "psiformer" 
...
parser.add_argument("--config_arg", type=str, required=False, help="Argument to be passed to get_config (integer or None).")
args = parser.parse_args()

# Convert the argument to an integer or None
if args.config_arg == "None":
    config_arg = None
else:
    try:
        config_arg = int(args.config_arg)
    except ValueError:
        raise ValueError("Argument must be an integer or None.")

cfg = get_config(config_arg)
train.train(cfg)