...
def make_LLL_envelope_2d_trainable_zeros_mixed_1LL(
    lattice: jnp.ndarray,
    elliptic_log_sigma=None,   # kept for API compatibility
    z_scale: float = 1.0,
    magfield_kwargs=None,
  ...
  def apply(
    ...
    def log_env_one_electron(z_e: jnp.ndarray) -> jnp.ndarray:
      ...
      if N_holo > 0 and zeros_h_c.shape[1] > 0:
        def log_holo_for_orb(zeros_k):
          return ellipticfunctions._1LL_with_zeros_log_cached(
              z_e, zeros_k, consts_holo
          )
        log_holo = jax.vmap(log_holo_for_orb, in_axes=0)(zeros_h_c)  # (odim,)
      else:
      ...
      if N_anti > 0 and zeros_a_c.shape[1] > 0:
        def log_anti_for_orb(zeros_k):
          val = ellipticfunctions._1LL_with_zeros_log_cached(
              jnp.conj(z_e), zeros_k, consts_anti
          )
          return jnp.conj(val)
        log_anti = jax.vmap(log_anti_for_orb, in_axes=0)(zeros_a_c)  # (odim,)
...
def make_LLL_envelope_2d_trainable_zeros_mixed6(
    lattice: jnp.ndarray,
    elliptic_log_sigma=None,   # kept for API compatibility
    z_scale: float = 1.0,
    magfield_kwargs=None,
  ...
  This version is more memory-efficient:
    * uses chunked vmap over electrons via lax.scan,
    * checkpoints the per-electron computation to reduce activation memory.
  """
  if magfield_kwargs is None:
    magfield_kwargs = {}

  # optional: how many electrons per chunk (0 or None → no chunking)
  env_chunk_size = int(magfield_kwargs.get("env_chunk_size", 0))
  ...
  consts_holo = jax.tree.map(lambda x: jax.lax.stop_gradient(jnp.asarray(x)),
                             consts_holo)
  consts_anti = jax.tree.map(lambda x: jax.lax.stop_gradient(jnp.asarray(x)),
                             consts_anti)
  ...
  # ---------- apply: chunked + vmapped + checkpointed ----------
  def apply(
      *,
      ae: jnp.ndarray,
      r_ae: jnp.ndarray,
    ...
    def log_env_one_electron(z_e: jnp.ndarray) -> jnp.ndarray:
      ...
      def log_for_orb(zeros_h_row, zeros_a_row):
        # zeros_h_row: (N_holo,), zeros_a_row: (N_anti,)
        # holomorphic part
        if N_holo > 0 and zeros_h_row.size > 0:
          log_holo = ellipticfunctions._LLL_with_zeros_log_cached(
              z_e, zeros_h_row, consts_holo
          )
        else:
          log_holo = jnp.array(0.0 + 0.0j, dtype=jnp.complex64)

        # anti-holomorphic part
        if N_anti > 0 and zeros_a_row.size > 0:
          val = ellipticfunctions._LLL_with_zeros_log_cached(
              jnp.conj(z_e), zeros_a_row, consts_anti
          )
          log_anti = jnp.conj(val)
        else:
          log_anti = jnp.array(0.0 + 0.0j, dtype=jnp.complex64)

        return log_holo + log_anti  # scalar complex

      # vmap over orbitals (axis 0: odim)
      # zeros_h_c: (odim, N_holo), zeros_a_c: (odim, N_anti)
      log_e = jax.vmap(log_for_orb, in_axes=(0, 0))(zeros_h_c, zeros_a_c)  # (odim,)
      return log_e

    # checkpoint per-electron to save memory in backward pass
    #log_env_one_electron_ckpt = jax.checkpoint(log_env_one_electron)

    # --- vmap / chunked vmap over electrons: (ne,) -> (ne, odim) ---

    if env_chunk_size and env_chunk_size > 0 and env_chunk_size < ne:
      # process electrons in chunks of env_chunk_size via lax.scan
      nchunks = (ne + env_chunk_size - 1) // env_chunk_size

      def scan_body(carry, idx):
        start = idx * env_chunk_size
        end = jnp.minimum(ne, (idx + 1) * env_chunk_size)
        z_chunk = jax.lax.dynamic_slice(z, (start,), (end - start,))
        log_chunk = jax.vmap(log_env_one_electron, in_axes=0)(z_chunk)
        carry = None
        return carry, log_chunk

      _, log_chunks = jax.lax.scan(
          scan_body,
          init=None,
          xs=jnp.arange(nchunks, dtype=jnp.int32),
      )
      log_env_eo = jnp.reshape(log_chunks, (ne, odim))
    else:
      # plain vmap over electrons
      log_env_eo = jax.vmap(log_env_one_electron, in_axes=0)(z)  # (ne, odim)

    # --- KFAC dense tagging (unchanged) ---
    if use_kfac_dense:
      # Holomorphic zeros
      if N_holo > 0 and zeros_holo_unconstrained.size > 0:
        in_dim_h = N_holo * 2
...
def make_LLL_envelope_2d_trainable_zeros_mixed7(
    lattice: jnp.ndarray,
    elliptic_log_sigma=None,   # kept for API compatibility
    z_scale: float = 1.0,
    magfield_kwargs=None,
  ...
  This version avoids *all* JAX control-flow primitives (no vmap, no scan, no fori_loop)
  inside `apply`, using plain Python loops instead. That plays nicer with folx and
  can be more memory friendly at the cost of some speed.
  """
  if magfield_kwargs is None:
    magfield_kwargs = {}
  ...
  # ---------- apply: NO vmap / NO scan / NO fori_loop ----------
  def apply(
      *,
      ae: jnp.ndarray,
      r_ae: jnp.ndarray,
    ...
    z = z_scale * (pos[..., 0] + 1j * pos[..., 1]) / jnp.sqrt(2.0)
    z = z.astype(jnp.complex64)  # (ne,)
    ne = ae.shape[0]             # static electron count
    odim = gain.shape[0]         # static orbital count
    ...
    # allocate log_env_eo (ne, odim)
    log_env_eo = jnp.zeros((ne, odim), dtype=jnp.complex64)

    # plain Python loops over electrons and orbitals
    # (these are unrolled at trace time since ne, odim are static)
    for i in range(ne):
      z_e = z[i]
      # build one row log_env_eo[i, :]
      row = []
      for m in range(odim):
        # holomorphic part
        if have_holo:
          zeros_h_row = zeros_h_c[m]  # (N_holo,)
          log_holo = ellipticfunctions._LLL_with_zeros_log_cached(
              z_e, zeros_h_row, consts_holo
          )
        else:
        ...
        # anti-holomorphic part
        if have_anti:
          zeros_a_row = zeros_a_c[m]  # (N_anti,)
          val = ellipticfunctions._LLL_with_zeros_log_cached(
              jnp.conj(z_e), zeros_a_row, consts_anti
          )
          log_anti = jnp.conj(val)
        ...
        row.append(log_holo + log_anti)
      row = jnp.stack(row, axis=0)  # (odim,)
      log_env_eo = log_env_eo.at[i].set(row)
